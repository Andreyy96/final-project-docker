...
class AuthService {
    ...
    public async signUp(): Promise<IUser> {
        const hashPassword = await passwordService.hashPassword("admin");
    ...
    public async activateAccountGetURL(
        ...
        return `localhost/activate/${token}`;
    }
    ...
    public async recoveryPasswordGetURL(email: string): Promise<string> {
        ...
        return `localhost/recovery-password/${token}`;
    }
    ...
    public async recoveryPasswordSet(
        ...
        const password = await passwordService.hashPassword(dto.password);
        await userRepository.updatePasswordById(jwtPayload.userId, {
            password,
        });
